...
cuicui.WIDGET_MODULE_STRUCT = {
    ...
    --- - Only layout properties (natural_width, natural_height, width, height, x, y) go in render_tree
    ---
    --- @param props_tree table: Map of widget IDs to their properties (forms a tree via parent/child references)
    --- @param render_tree table: Map of widget IDs to their layout properties (generated by layout algorithm)
    --- @param id any: The ID key for this widget in the props_tree and render_tree
    ...
    --- @param id any: The ID key for this widget in the props_tree and event_tree
...
local function make_props(tree, key, parent_id, widget)
    ...
    widget.populate_default_props(props, old_props, tree.event[props.id])
...
local function make_ui(tree, parent_props)
    ...
    local function index(_, key)
            ...
            for state in pairs(cuicui.EVENT_STATES) do
                if key == state then
                    if tree.event[parent_props.id] then
                        return tree.event[parent_props.id][state]
                    else
                        return nil
                    end
                end
        ...
        return function(fn)
            ...
            tree.event[props.id] = tree.event[props.id] or {}
...
local function index_cuicui(_, key)
    ...
    return function(term, fn)
        ...
        while true do
            ...
            tree.event[root_props.id] = tree.event[root_props.id] or {}
